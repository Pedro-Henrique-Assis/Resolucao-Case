...
class ColaboradorResourceTest {
    ...
    class cadastrarColaboradorEntity {
        ...
        void deveCriarColaborador() throws Exception {
            ...
            when(colaboradorService.cadastrarColaborador(any(ColaboradorCadastroRequest.class)))
                    .thenReturn(matriculaGerada);

            // Cria o DTO simulando o input do usuário
            var dto = new ColaboradorCadastroRequest("Alice", LocalDate.of(2024,1,1), "Engenheira");

            // Captura do DTO enviado ao service
            ArgumentCaptor<ColaboradorCadastroRequest> dtoCaptor = ArgumentCaptor.forClass(ColaboradorCadastroRequest.class);

            // Act
            // Executa o POST /api/v1/colaborador com o JSON do DTO
            mockMvc.perform(
                    ...
                    .andExpect(header().string("Location", "/api/v1/colaborador/" + matriculaGerada));
            ...
            ColaboradorCadastroRequest capturado = dtoCaptor.getValue();
            assertEquals("Alice", capturado.nome());
            assertEquals(LocalDate.of(2024,1,1), capturado.dataAdmissao());
            assertEquals("Engenheira", capturado.cargo());
        }
    }
    ...
    class consultarColaboradorPorMatriculaEntity {
        ...
        void deveRetornar200QuandoEncontrado() throws Exception {
            ...
            var dto = new ColaboradorResponse(
                    matricula,
                    "Bob",
            ...
            // Act
            // Executa GET /api/v1/colaborador/{matricula}
            mockMvc.perform(get("/api/v1/colaborador/{matricula}", matricula))
                    // Assert
    ...
    class listarColaboradores {
        ...
        void deveListarCom200() throws Exception {
            // Arrange
            var c1 = new ColaboradorResponse(UUID.randomUUID(),"Ana", LocalDate.of(2024,1,1), "Dev", null, List.of());
            var c2 = new ColaboradorResponse(UUID.randomUUID(),"Caio", LocalDate.of(2024,1,2), "QA", null, List.of());

            // Configura o Mock para retornar a lista
            when(colaboradorService.listarColaboradores()).thenReturn(List.of(c1, c2));

            // Act + Assert
            mockMvc.perform(get("/api/v1/colaborador"))
                    .andExpect(status().isOk())
                    .andExpect(content().contentType(MediaType.APPLICATION_JSON))
    ...
    class calcularPerformanceFinal {
        ...
        void deveRetornar200ComPerformance() throws Exception {
            ...
            var medias = new ColaboradorMediaPerformanceResponse(
                    new BigDecimal("8.25"),
                    new BigDecimal("9.00"),
                    new BigDecimal("17.25")
            );
            var perf = new ColaboradorPerformanceResponse(matricula, "Alice", medias);

            // Configura o Mock para retornar o DTO
            when(colaboradorService.calcularPerformanceFinal(matricula.toString())).thenReturn(perf);

            // Act + Assert
            mockMvc.perform(get("/api/v1/colaborador/{matricula}/performance", matricula))
                    .andExpect(status().isOk())
                    .andExpect(content().contentType(MediaType.APPLICATION_JSON))
                    .andExpect(jsonPath("$.matricula").value(matricula.toString()))
    ...
    class deletarColaboradorPorMatriculaEntity {
        ...
        void deveDeletarCom204() throws Exception {
            ...
            // Act + Assert
            mockMvc.perform(delete("/api/v1/colaborador/{matricula}", matricula))
    ...
    @Nested
    class atualizarColaboradorPorMatriculaEntity {
        ...
        void deveAtualizarCom204() throws Exception {
            ...
            // Cria o DTO simulando o input do usuário
            var dto = new ColaboradorAtualizaRequest("Novo Nome", LocalDate.of(2023,1,1), "Novo Cargo");

            // Captura dos argumentos enviados ao service
            ArgumentCaptor<String> matriculaCaptor = ArgumentCaptor.forClass(String.class);
            ArgumentCaptor<ColaboradorAtualizaRequest> dtoCaptor = ArgumentCaptor.forClass(ColaboradorAtualizaRequest.class);

            doNothing().when(colaboradorService).atualizaColaboradorPorMatricula(anyString(), any(ColaboradorAtualizaRequest.class));

            // Act
            // Executa PATCH /api/v1/colaborador/{matricula} com JSON
            mockMvc.perform(
                            patch("/api/v1/colaborador/{matricula}", matricula)
                                    .contentType(MediaType.APPLICATION_JSON)
            ...
            assertEquals(matricula.toString(), matriculaCaptor.getValue());
            assertEquals("Novo Nome", dtoCaptor.getValue().nome());
            assertEquals(LocalDate.of(2023, 1, 1), dtoCaptor.getValue().dataAdmissao());
            assertEquals("Novo Cargo", dtoCaptor.getValue().cargo());
        }
    }
}